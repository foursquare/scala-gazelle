...
This repo is actively used and maintained with the ultimate goal of upstreaming it to
[rules_scala](https://github.com/bazelbuild/rules_scala). Contributions and bug reports
are welcome here with the understanding that they will be handled on a best effort basis,
with priority given to those which advance towards this goal.

TODO: cross-link tracking issue once it exists

## Requirements

 - A maven install lockfile generated by [rules_jvm_external](https://github.com/bazelbuild/rules_jvm_external),
  or some other json file mapping 3rdparty jars to the list of code packages they provide
  which matches its format.
...
This plugin has not been tested on a Scala 3 codebase. While the underlying
[tree-sitter](https://github.com/tree-sitter/tree-sitter-scala) parsing library it uses
does support Scala 3, it is very likely the plugin itself may experience crashes or
produce erroneous output when run over a Scala 3 codebase. In such cases, it may be
possible to work around bugs via `#gazelle:ignore` and `#gazelle:resolve` directives with
some effort.
